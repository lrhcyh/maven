...
$("#oneStatis").click(function(){

	$("#tabs").tabs("select",0);
	$("#statisPostBox").window("open");
...
function saveOne(){
	...
	var name=$('#oneName').val();
	 var text=$("#twoText").attr('val');
	 if(text==null||text==''){
		 $.messager.alert('info','请添加保存项');
		 return ;
	 }
	 $.ajax({
			type : 'POST',
			url : 'statistics/saveOne.form',
			data:{'name':name,'text':text,'modulecode':'6'},
			dataType : 'text',
			async:false ,
...
function saveTwoName(){
	...
	var content="<p val='"+$("#twoText").attr('val')+"' title='"+$("#twoText").val()+"'>"+$("#twoText").attr('aaa')+"</p>";
	$("#twoText").val('');
	  ...
	  onSelect: function(title,index){
		   ...
		   if (index==1 && tabType2==0){
			   ...
			   $('#twoCartogram').datagrid({
               	url : 'statistics/findTwoCondition.form?moduleCode=6'
               });
			   tabType2=1;
			   excelType=2;
		   }else{
