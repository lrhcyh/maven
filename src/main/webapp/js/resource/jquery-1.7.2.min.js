...
/**
 * ajax请求的方法
 * @arguments url,map, callback, errorCallBack
 */
function ajaxFunction(_url_,ajax_map_,callBack,errorCallBack) {
	var argNum=arguments.length;
	if(argNum==0){
		alert("ajax调用异常！");
		return;
	}
	var ajax_data_type=ajax_map_.dataType&&ajax_map_.dataType!=null?ajax_map_.dataType:"json";
	var ajax_async=ajax_map_.async==false?false:true;
	delete ajax_map_.dataType;
	delete ajax_map_.async;
	jQuery.ajax({
		url : _url_,
		...
		async : ajax_async,
		type : "post",
		error : argNum>=4?errorCallBack:ajaxErrorFunction,
		success : argNum>=3?callBack:function(){}
	});
}
function ajaxErrorFunction(XMLHttpRequest, textStatus, errorThrown) {
	var text=XMLHttpRequest.responseText;
	if (text!=""&&text.indexOf('Session has expired,Please login again')!=-1) {
		userLoginCallBack();
	}else if(text!=""){
		//jInfo("参数有误!");
		//$("body").text(text);
...
function ajaxNoLogin(XMLHttpRequest, textStatus, errorThrown) {
	ajaxErrorFunction(XMLHttpRequest, textStatus, errorThrown);
}
